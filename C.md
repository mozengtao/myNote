...
```c
int main() {
    UART_Device *uart1 = uart_device_create(UART1, 115200);
    if(!uart) {
        return -1;
    }

    UART_Device *uart2 = uart_device_create(UART2, 115200);
    if(!uart2) {
        return -1;
    }

    uart_device_send(uart1, "Hello, World!", 13);
    uart_device_recv(uart1, 10);

    uart_device_send(uart2, "Hello, World!", 13);
    uart_device_recv(uart2, 10);

    uart_device_destroy(uart1);
    uart_device_destroy(uart2);
