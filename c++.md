...
```
项目概述
	本方案为有5年C经验并已学C++基础的程序员设计，旨在通过系统分析LevelDB源码，掌握现代C++在大型项目中的实际应用。

学习目标
	深入理解LevelDB架构设计和实现原理
	掌握C++高级特性在工程中的实际应用
	完成从C到C++编程思维的转变
	具备分析复杂C++项目的能力
```
...
**提示词**1：项目整体分析
请为LevelDB项目生成架构概览文档，包含：
	1. **项目架构图**：绘制LevelDB的核心组件关系图
	2. **代码组织结构**：分析include/、db/、table/、util/等目录的职责划分
	3. **构建系统**：分析Makefile和构建流程
	4. **核心设计理念**：LSM-Tree、WAL、SSTable等关键概念解释

要求：结合C++项目特点，对比传统C项目的架构差异，突出面向对象设计的好处。
输出格式：Markdown文档，包含代码目录树和组件交互图。

- 第二阶段：从C到C++的代码对比学习

**提示词**2：内存管理对比分析
分析LevelDB中的内存管理机制，重点对比C与C++的不同实现方式：
	1. **RAII模式应用**：找出至少3个使用RAII的类，说明其资源管理方式
	2. **智能指针使用**：分析unique_ptr、shared_ptr的使用场景和原因
	3. **对比传统C实现**：如果用C实现类似功能，内存管理会如何设计

请提供具体代码文件、行号和示例，说明每种模式的优缺点。

**提示词**3：错误处理机制分析
分析LevelDB的Status类设计：
	1. **类定义分析**：Status.h/cpp的完整实现解析
	2. **错误码设计**：对比C语言错误码处理的差异
	3. **使用模式**：在代码库中找出5个典型的使用场景
	4. **性能考量**：返回值优化(RVO)、移动语义的应用

要求：给出从C错误处理迁移到Status类的具体示例和最佳实践。

- 第三阶段：核心数据结构深入学习

**提示词**4：SkipList实现分析
深入分析db/skiplist.h和db/skiplist.cc：
	1. **模板设计**：分析SkipList的模板参数设计思想
	2. **并发控制**：内存屏障、原子操作的使用
	3. **迭代器模式**：Iterator接口的设计与实现
	4. **性能优化**：对比传统平衡树的优势

请绘制SkipList的内存布局图，并分析插入、查找、删除的时间复杂度。

**提示词**5：Slice字符串设计
分析include/leveldb/slice.h的设计：
	1. **零拷贝设计**：解释Slice如何避免字符串拷贝
	2. **API设计**：对比std::string的接口设计差异
	3. **内存安全**：生命周期管理的注意事项
	4. **实际应用**：在代码库中找出3个典型使用场景
...
**提示词**6：LSM-Tree实现分析
分析LevelDB的LSM-Tree完整实现：
	1. **MemTable**：内存表结构与刷新机制
	2. **SSTable**：磁盘文件格式与读取优化
	3. **Compaction**：合并策略与性能平衡
	4. **版本控制**：VersionSet的管理机制

请绘制数据写入和读取的完整流程图，标注关键代码文件。

**提示词**7：文件格式与编码
分析table/目录下的文件格式实现：

1. **Block格式**：data block、index block的设计
2. **Bloom Filter**：布隆过滤器的实现与应用
3. **变长编码**：Varint等编码技术的使用
4. **校验和**：CRC校验的实现

要求：提供二进制文件格式的详细解析和示例。
- 第五阶段：高级C++特性应用

**提示词**8：模板与泛型编程
分析LevelDB中模板的高级应用：

1. **Comparator模板**：泛型比较器的设计
2. **Iterator模板**：迭代器模式的泛型实现
3. **类型萃取**：类似std::enable_if的技术应用
4. **策略模式**：通过模板实现的策略模式

请找出3个模板元编程的典型案例并详细解释。

**提示词**9：并发编程模式
分析LevelDB的并发控制机制：

1. **锁设计**：Mutex、RWLock的实现与使用
2. **无锁编程**：原子操作和内存序的使用
3. **线程模型**：后台压缩线程的设计
4. **性能优化**：热点路径的锁优化策略

要求：对比C语言中类似并发控制的实现差异。
- 第六阶段：实践与代码分析

**提示词**10：代码阅读路线图
为LevelDB生成一个14天的代码阅读计划：

**每日学习任务**：
- 第1-2天：基础组件（Slice、Status、Env）
- 第3-4天：内存管理（Arena、Cache）
- 第5-6天：核心数据结构（SkipList、Comparator）
- 第7-8天：存储引擎（DBImpl、VersionSet）
- 第9-10天：文件格式（TableBuilder、Block）
- 第11-12天：高级特性（Iterator、Filter）
- 第13-14天：整体联调与性能分析

每个任务包含：重点阅读文件、关键函数、学习目标、实践练习。
实践项目提示词

**提示词**11：模仿实现练习
基于LevelDB的设计，指导我实现一个简化版的KV存储：
	**分阶段实现**：
	1. 基础Slice和Status类
	2. SkipList内存表
	3. 简单的文件存储
...
每个阶段提供：接口设计建议、关键实现要点、测试方法。

- 学习效果验证
**提示词**12：知识检验
为LevelDB学习生成测试题目：
	**理解题**：
	1. 解释LSM-Tree的读写放大问题及LevelDB的解决方案
	2. 分析Status类相比C错误码的优势

	**实践题**：
	1. 如何扩展LevelDB支持新的Comparator
	2. 优化MemTable刷新策略的设计思路

	**设计题**：
	1. 如果用C重新实现LevelDB，架构会有何不同
	2. 如何为LevelDB添加事务支持

Cursor学习规则配置
	创建.cursorrules文件：
	用户背景：5年C经验，C++基础，学习LevelDB进阶
	学习重点：
	- C++特性在工程中的实际应用
	- 设计模式与架构设计
	- 从C到C++的思维转变
...
- 进阶学习建议
完成基础学习后，可继续以下方向：
	性能优化：分析LevelDB的性能瓶颈和优化策略
	扩展功能：研究如何添加新特性（如压缩、加密）
	对比学习：与其他KV存储（RocksDB、SQLite）对比
	生产实践：学习LevelDB在实际项目中的应用案例

通过本系统学习方案，你将能够深入掌握LevelDB的设计精髓和C++在大型项目中的工程实践
